...
def graphics(data):
    ...
    del table_dn['Razón Social de la SGR']
    fig, ax = plt.subplots(1, 1)
    fig.set_size_inches(5, 5)
    colors = ['#99ff99', '#66b3ff', '#99ff99', '#ffcc99']
    # explode = (0, 0.1,)
    table_dn.loc['Total'].plot(kind='pie', ax=ax, colors=colors, autopct='%1.1f%%')
    ax.set_ylabel('Relación de montos')
    # ax.set_xlabel('cpd')
    fig.suptitle('Montos - CPD', fontsize=20)
    fig.savefig('img/montos_cpd.jpg')
    # plt.show()
    ...
    fig, ax = plt.subplots(1, 1)
    fig.set_size_inches(5, 5)
    colors = ['#99ff99', '#eecf22']
    # explode = (0, 0.1,)
    table_dn.loc['Total'].plot(kind='pie', ax=ax, colors=colors, autopct='%1.1f%%')
    ax.set_ylabel('Relación de montos')
    # ax.set_xlabel('cpd')
    fig.suptitle('Montos - ECHEQ', fontsize=20)
    fig.savefig('img/montos_echeq.jpg')
    # plt.show()
    ...
    colors = ['#22ff99', '#eecf22', '#aac522']
    # explode = (0, 0.1,)
    table_dn['Desierta'].plot(kind='pie', ax=ax, colors=colors, autopct='%1.1f%%')
    ax.set_ylabel('Relación de montos')
    # ax.set_xlabel('cpd')
    fig.suptitle('Montos - PAGARE', fontsize=20)
    fig.savefig('img/montos_pag.jpg')
    # plt.show()
    ...
    # #######################################################
    # Create pivot table DESIERTA - NEGADA
    table_dn = data.pivot_table(index='Razón Social de la SGR', columns='Estado', values='Monto', aggfunc=np.sum) \
        .reset_index()
    # Erase Desierta (Baja) column
    table_dn.drop(table_dn.columns[2], axis=1, inplace=True)
    table_dn.columns.name = None
    ...
    ax.set_ylabel('Relación de montos')
    # ax.set_xlabel('cpd')
    fig.suptitle('Montos - Tipo Cheque: Todos', fontsize=20)
    fig.savefig('img/montos_todos.jpg')
    # plt.show()
...
def add_cheque_tohtml_bavsa(data):
    ...
    """
    # Static = bavsa
    with open('tables/bavsa_html.html', 'r') as f:
        static = str(f.read())
...
def create_html_bavsa(file_name, table_tag):
    ...
    if file_name == "negociada_CPD":
        ...
        with open('tables/template3.html', 'r') as f:
            static = str(f.read())
    else:
        # Static = bavsa
        with open('tables/bavsa_html.html', 'r') as f:
...
def main():
    # Fix file
    const_file = 'src/subastas.xlsx'
    file_path = None

    if not (len(sys.argv) == 2):
        print('This program needs the file name')
        print('example: python main.py subastas')
        sys.exit(1)  # Exit program because of error
    try:
        file_name = str(sys.argv[1])
        file_path = 'src/' + file_name + '.xlsx'
        # Read excel file with pandas (skip some rows)
        data = pd.read_excel(file_path, skiprows=range(0, 3))
    except FileNotFoundError:
        print('No such file of directory: ', file_path)
        print('Your file (' + file_path + ') is replace with: ', const_file)
        file_path = const_file
        data = pd.read_excel(file_path, skiprows=range(0, 3))
    ...
    copyfile('tables/bavsa_html.html', 'web2/bavsa.html')
